...
const content_mapping = content.reduce((obj, x) => {
  obj[x.slug] = x;
  return obj;
}, {});
...
app.get("/", (req, res) => res.render("blog_index.html", { posts: content }));

app.get("/feed", (req, res) => {
  const lastupdate = content.reduce((x, y) => (x.updated > y.updated ? x : y));
  res.set("content-type", "application/atom+xml");
  return res.render("blog_feed.xml", { posts: content, lastupdate });
});

app.get("/:name", ({ params: { name } }, res) => {
  if (name in content_mapping) {
    return res.render("blog_post.html", {
      post: content_mapping[name]
    });
  }
});
